...
def addtocart(request,id):
    ...
    if cart_id:
       ...
       cartproduct = CartProduct.objects.create(cart=cart_item, product
       =cart_product, rate = cart_product.price, quantity=1, subtotal=cart_product.price)
       cart_item.total += cart_product.price
       cart_item.save()
...
def emptyCart(request):
    ...
    if cart_id:
        cart = Cart.objects.get(id=cart_id)
        # assign to cart
        if request.user.is_authenticate and request.user.customer:
            cart.customer = request.user.customer
            cart.save()
        #end 
...
def orderDetails(request,id):
    ...
    orders = Order.objects.get(get=id)
    context = {
       'orders': orders,
   }   
...
def search(request):
    ...
    results = Product.objects.filter(Q(title__icontains=thekw) |
    Q(description__icontains=thekw)) 
    context = {
       'results': results,
   }   
